...
class BrainPipeline(object):
    ...
    def norm_slices(self):
        '''
        normalizes each slice in self.slices_by_slice, excluding gt hoise na? hmm (y) dekhla to? bakita kore felo! :P  ki korbo? ami ki korbo janina to! sob ula thik koro! ses! ar kichu na!
        subtracts mean and div by std dev for each 
...
def save_labels(fns):
    ...
    '''
    for label_idx in xrange(len(labels)):
